...
public class UIThemeController : MonoBehaviour
{
    ...
    [SerializeField] private OptionBarWrapperController[] _optionBarWrappers;
    ...
    public void ApplyTheme()
    {
        ...
        if (_windowContainers != null)
            foreach (var c in _windowContainers) c?.ApplyTheme(_theme);

        if (_multiPaneControllers != null)
            foreach (var c in _multiPaneControllers) c?.ApplyTheme(_theme);

        if (_leftTabPaneControllers != null)
            foreach (var c in _leftTabPaneControllers) c?.ApplyTheme(_theme);

        if (_tabbedConsoleControllers != null)
            foreach (var c in _tabbedConsoleControllers) c?.ApplyTheme(_theme);

        if (_optionBarWrappers != null)
            foreach (var c in _optionBarWrappers) c?.ApplyTheme(_theme);

        if (_fileHierarchyControllers != null)
            foreach (var c in _fileHierarchyControllers) c?.ApplyTheme(_theme);

        if (_consoleRenderers != null)
            foreach (var r in _consoleRenderers) r?.ApplyTheme(_theme);
    }
    ...
    private void CacheMissingReferences()
    {
        ...
        if (_optionBarWrappers == null || _optionBarWrappers.Length == 0)
            _optionBarWrappers = FindObjectsByType<OptionBarWrapperController>(FindObjectsInactive.Include, FindObjectsSortMode.None);
